
<?php 
session_start();
$server="localhost";
$username="root";
$password="";
$database="3rs";

$con=mysqli_connect($server,$username,$password,$database);
$sql= "SELECT * from feedback where 1";

$result=mysqli_query($con,$sql);

?>


<!DOCTYPE html>
<html >
  <head>
  <link rel="stylesheet" href="employee.css">

</head>
<body>
    
    

<?php include"sidbar.php"; ?>     

    <div class="main-content">
      <header>
        <h2>
          <label for="nav-toggle">
            <span class="fas fa-bars"></span>
          </label>
         Employee
        </h2>

        
        <div class="user-wrapper">
         <img src="data:image/png;base64," width="40px" height="40px" alt="profile-img">
         <div class="">
            <h4>Hi <?php echo $_SESSION["name"] ?></h4>
            <small> Employee</small>
         </div>
        </div>
  
      </header>

      <main>
       

        <div class="recent-grid">
          <div class="projects">
            <div style="width: 950px;" class="card">
              <div class="card-header">
                <h2>Feedback</h2>
               
              </div>
              <div class="card-body">
                <div class="table-responsive">
                  <table width="100%">
                  <thead>
                    <tr>
                      <td>#</td>
                      <td>message</td>
                      <td>email	</td>
                      <td>Status</td>
                 
                      <td>Delete</td>

                    </tr>
                  </thead>
                  <tbody>
                  <?php foreach ($result as $row) {   ?>
                    <tr>
                      <td><?php echo $row['id'];?></td>
                      <td><?php echo $row['message'];?></td>
                      <td><?php echo $row['email'];?></td>	
                     	
												<td>		<?php
														if($row['status']=="Accept"){ ?>
										<div style="background-color:#96b851;border-radius:20px;border:#fff 1px solid">	<a style="color:#fff;font-size:15px;margin-left:35px;"  href="update_feedback.php?id=<?php echo $row['id']; ?>" ><?php echo $row['status'];?>  </a>	</div>
													<?php	}
														else {
														
														?>
														<div style="background-color:blue;border-radius:20px;border:#fff 1px solid">	 <a style="color:#fff;font-size:15px;margin-left:35px;" href="update_feedback.php?id=<?php echo $row['id'];  ?>" class="btn btn-primary"><?php echo $row['status'];?>  </a></div>
														<?php } ?></td>
                       <td><a href="delete_feedback.php?id=<?php echo $row['id'];  ?> "class="btn btn-danger"> Delete</a></td>
                    </tr>
                    <?php } ?>
            
                  </tbody>

                </table>
                </div>
              </div>

            </div>

          </div>
       
          
        </div>  

      </main>
    </div>


</body>
</html>

