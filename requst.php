
<?php 
session_start();
$server="localhost";
$username="root";
$password="";
$database="3rs";

$con=mysqli_connect($server,$username,$password,$database);
$sql= "SELECT * from request where 1";
// $result=$con->query($sql);
$result=mysqli_query($con,$sql);

?>


<!DOCTYPE html>
<html >
  <head>
  <link rel="stylesheet" href="employee.css">

</head>
<body>
    
    

<?php include"sidbar.php"; ?>     

    <div class="main-content">
      <header>
        <h2>
          <label for="nav-toggle">
            <span class="fas fa-bars"></span>
          </label>
          Employee
        </h2>

      

        <div class="user-wrapper">
         <img src="data:image/png;base64," width="40px" height="40px" alt="profile-img">
         <div class="">
            <h4>Hi <?php echo $_SESSION["name"] ?></h4>
            <small> Employee</small>
         </div>
        </div>
      </header>

      <main>
       

        <div class="recent-grid">
          <div class="projects">
            <div style="width: 950px;" class="card">
              <div class="card-header">
                <h2>Request</h2>
          
              </div>
              <div class="card-body">
                <div class="table-responsive">
                  <table width="100%">
                  <thead>
                    <tr>
                      <td>#</td>
                      <td>Kind</td>
                    
                      <td>Weight</td>
                      <td>User Id</td>
                      <td>Status</td>
                   
                      <td>Delete</td>

                    </tr>
                  </thead>
                  <tbody>
                  <?php foreach ($result as $row) {   ?>
                    <tr>
                      <td><?php echo $row['id'];?></td>
                      <td><?php echo $row['kind'];?></td>
                    
                      
                      <td><?php echo $row['weight'];?></td>
                      <td><?php echo $row['user_id'];?></td>
                      <td>		<?php
														if($row['status']=="Accept"){ ?>
										<div style="background-color:#96b851;width:130px;border-radius:20px;">	<a style="color:#fff;font-size:15px;margin-left:35px;"  href="update_request.php?id=<?php echo $row['id']; ?>" ><?php echo $row['status'];?>  </a>	</div>
													<?php	}
														else {
														
														?>
														<div style="background-color:blue;width:130px;border-radius:20px;">	 <a style="color:#fff;font-size:15px;margin-left:35px;" href="update_request.php?id=<?php echo $row['id'];  ?>" class="btn btn-primary"><?php echo $row['status'];?>  </a></div>
														<?php } ?></td>
                       <td><a href="delete_requst.php?id=<?php echo $row['id'];  ?> "class="btn btn-danger"> Delete</a></td>
                    </tr>
                    <?php } ?>
            
                  </tbody>

                </table>
                </div>
              </div>

            </div>

          </div>
       
          
        </div>  

      </main>
    </div>


</body>
</html>

